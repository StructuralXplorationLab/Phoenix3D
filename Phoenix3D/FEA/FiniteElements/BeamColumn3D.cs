...
namespace Cardinal.FEA
{
    class BeamColumn3D : FiniteElement
    {
        internal BeamColumn3D(ref IMember1D M)
        {
            SetMemberStiffnessMatrices(ref M);
            SetDofs(ref M);
            SetGlobalElementStiffnessMatrix(ref M);
        }

        private MatrixDense k0 = new MatrixDense(12);
        private MatrixDense T = new MatrixDense(12);
        internal override int[] Dofs { get; set; } = new int[12];
        internal override MatrixDense K { get; set; }

        internal override void SetDofs(ref IMember1D M)
        {
            for (int i = 0; i < 6; i++)
            {
        ...
        internal override void SetGlobalElementStiffnessMatrix(ref IMember1D Member)
        {
            K = T.Transpose() * k0 * T;
        }
        internal override Vector GetLocalDisplacements(Vector u, ref IMember1D Member)
        {
        ...
        private void SetMemberStiffnessMatrices(ref IMember1D Member)
        {
            var TLocal = Member.T;

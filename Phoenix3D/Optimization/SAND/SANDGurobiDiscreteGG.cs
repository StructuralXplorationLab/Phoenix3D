...
namespace Phoenix3D.Optimization.SAND
{
    ...
    public static class SANDGurobiDiscreteGG
    {
        ...
        public static Dictionary<LoadCase, GRBVar[]> GetGurobiMemberStressVariables(GRBModel Model, Structure Structure, List<LoadCase> LoadCases)
        {
            ...
            foreach (LoadCase LC in LoadCases)
            {
                GRBVar[] StressesLC = new GRBVar[Structure.Members.OfType<IMember1D>().Count()];
                foreach(IMember M in Structure.Members)
                {
                    StressesLC[M.Number] = Model.AddVar(-M.Material.fc, M.Material.ft, 0, GRB.CONTINUOUS, "StressLC" + LC.Name + "Member" + M.Number.ToString());
                }
                MemberStresses.Add(LC, StressesLC);
            }
            return MemberStresses;
        }
            ...
            foreach(Node N in Structure.Nodes)
            {
                for(int d = 0; d < 3; d++)
                {
                    ...
                    foreach(IMember1D M in N.ConnectedMembers.Keys)
                    {
                        ...
                        f.AddTerm(N.ConnectedMembers[M] * M.Direction[d] , MemberForces[M.Number]);
                    }
        ...
        public static void AddConstitutive(GRBModel model, GRBVar[] MemberForces, GRBVar[] MemberElongations, Structure Structure, LoadCase LC, Stock Stock)
        {
            foreach(IMember1D M in Structure.Members)
            {
                GRBLinExpr sumAV = new GRBLinExpr();
                for(int j = 0; j < Stock.ElementGroups.Count; j++)
                {
                    sumAV.AddTerm(M.Material.E * Stock.ElementGroups[j].CrossSection.Area / M.Length, MemberElongations[M.Number * Stock.ElementGroups.Count + j]);
                }
                model.AddConstr(MemberForces[M.Number], '=', sumAV, "ConstitutiveLC" + LC.Name + "Member" + M.Number.ToString());
            }
        }
        public static void AddHooke(GRBModel model, GRBVar[] MemberStresses, GRBVar[] MemberElongations, Structure Structure, LoadCase LC, Stock Stock)
        {
            foreach (IMember1D M in Structure.Members)
            {
                ...
                for (int j = 0; j < Stock.ElementGroups.Count; j++)
                {
                    if(Stock.ElementGroups[j].CrossSection.Area > 0)
                        sumV.AddTerm(M.Material.E / M.Length, MemberElongations[M.Number * Stock.ElementGroups.Count + j]);
                }
                model.AddConstr(MemberStresses[M.Number], '=', sumV, "HookeLC" + LC.Name + "Member" + M.Number.ToString());
            }
        }
        public static void AddBigM(GRBModel model, GRBVar[] T, GRBVar[] MemberElongations, Structure Structure, LoadCase LC, Stock Stock, OptimOptions Options)
        {
            foreach (IMember1D M in Structure.Members)
            {
                for (int j = 0; j < Stock.ElementGroups.Count; j++)
                {
                    double emin;
                    double emax;
                    GetEminEmax(M, Stock.ElementGroups[j], out emin, out emax);

...
namespace Phoenix3D.Model.CrossSections
{
    public class SHSection : ICrossSection
    {
        ...
        public SHSection(double Width, double Thickness)
        {
            ...
            this.Name = "SHS " + (1000* Width).ToString() + "x" + (1000*Thickness).ToString();
        ...
        {
            bool found = false;
            string[] csvstring = Properties.Resources.SHSections.Split(new char[] { '\r', '\n' }, StringSplitOptions.RemoveEmptyEntries);
            for (int i = 1; i < csvstring.Length; i++)
            {
                string L = csvstring[i];
                string[] parts = L.Split(';');

                if (Convert.ToDouble(parts[1], new System.Globalization.CultureInfo("en-US")) == this.Height && Convert.ToDouble(parts[3], new System.Globalization.CultureInfo("en-US")) == this.Thickness)
                {
                    Area = double.Parse(parts[4], new System.Globalization.CultureInfo("en-US"));
                    ct = double.Parse(parts[5], new System.Globalization.CultureInfo("en-US"));
                    Iy = double.Parse(parts[6], new System.Globalization.CultureInfo("en-US"));
                    iy = double.Parse(parts[7], new System.Globalization.CultureInfo("en-US"));
                    Wy = double.Parse(parts[8], new System.Globalization.CultureInfo("en-US"));
                    Wypl = double.Parse(parts[9], new System.Globalization.CultureInfo("en-US"));
                    Iz = Iy;
                    Wz = Wy;
                    Wzpl = Wypl;
                    iz = iz;
                    It = double.Parse(parts[10], new System.Globalization.CultureInfo("en-US"));
                    Wt = double.Parse(parts[11], new System.Globalization.CultureInfo("en-US"));
                    Surface = double.Parse(parts[12], new System.Globalization.CultureInfo("en-US"));
                    cy = Width / 2;
                    cz = Height / 2;
                    Avy = Avz = 2 * Width * Thickness;
                    found = true;
                    break;
                }
            ...
            if (!found)
            {
                throw new System.Exception("No SHS-Section with width and height of " + (Width*1000).ToString() + " mm and thickness " + (Thickness*1000).ToString() + " mm could be found in the standard table");
            }
        }

...
namespace Phoenix3D.Model.CrossSections
{
    ...
    public class RHSection : ICrossSection
    {
        ...
        public RHSection(double Height, double Width, double Thickness)
        {
            ...
            this.Name = "RHS " + (1000 * Height).ToString() + "x" + (1000* Width).ToString() + "x" + (1000*Thickness).ToString();
        ...
        {
            bool found = false;
            string[] csvstring = Properties.Resources.RHSections.Split(new char[] { '\r', '\n' }, StringSplitOptions.RemoveEmptyEntries);
            for (int i = 1; i < csvstring.Length; i++)
            {
                ...
                if (Convert.ToDouble(parts[1], new System.Globalization.CultureInfo("en-US")) == this.Height && Convert.ToDouble(parts[2], new System.Globalization.CultureInfo("en-US")) == this.Width && Convert.ToDouble(parts[3], new System.Globalization.CultureInfo("en-US")) == this.Thickness)
                {
                    ...
                    cty = double.Parse(parts[5], new System.Globalization.CultureInfo("en-US"));
                    ctz = double.Parse(parts[6], new System.Globalization.CultureInfo("en-US"));
                    Iy = double.Parse(parts[7], new System.Globalization.CultureInfo("en-US"));
                    Iz = double.Parse(parts[8], new System.Globalization.CultureInfo("en-US"));
                    ...
                    Wypl = double.Parse(parts[13], new System.Globalization.CultureInfo("en-US"));
                    Wypl = double.Parse(parts[14], new System.Globalization.CultureInfo("en-US"));
                    It = double.Parse(parts[15], new System.Globalization.CultureInfo("en-US"));
                    Wt = double.Parse(parts[16], new System.Globalization.CultureInfo("en-US"));
                    Surface = double.Parse(parts[17], new System.Globalization.CultureInfo("en-US"));
                    cy = Width / 2;
                    cz = Height / 2;
                    Avy = 2 * Width * Thickness;
                    Avz = 2 * Height * Thickness;
                    found = true;
                    break;
                }
            ...
            if (!found)
            {
                throw new System.Exception("No RHS-Section with the given dimension could be found in the standard table");
            }
        }
